...
function checkExpression(exp, text, warn, range) {
    exp = typescript.transpileModule(`return ${exp}`, {
        compilerOptions: {
            module: typescript.ModuleKind.ESNext,
            target: typescript.ScriptTarget.ESNext,
        },
    }).outputText.trim()
    // console.log(exp, 'expexp')
    try {
        new Function(exp);
    }
    catch (e) {
...
const createCompiler$1 = createCompilerCreator(function baseCompile(template, options) {
    ...
    code.render = typescript.transpileModule(`${code.render}`, {
        compilerOptions: {
            module: typescript.ModuleKind.ESNext,
            target: typescript.ScriptTarget.ESNext,
        },
    }).outputText.trim()
    // console.log(ast, 1111, code.render, 2222)
    return {
        ast,
        render: code.render,
